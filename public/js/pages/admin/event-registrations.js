...
function buildRegistrationTableRows(registrations) {
  return registrations
    .map((reg) => {
      ...
      const event = allEvents.find((e) => e.event_id === reg.event_id) || {
        title: "Unknown",
      };
      const formattedDate = formatDate(reg.registration_date);

      const isAttended = reg.attended ? "checked" : "";
      const isMissed = reg.status === "missed";
      const isCancelled = reg.status === "cancelled";
      const disabledAttr = isMissed || isCancelled ? "disabled" : "";
      const titleAttr = isMissed
      ...
      <tr>
        ...
        <td>
          <input type="checkbox" class="attendance-checkbox" data-id="${reg.registration_id}" ${isAttended} ${disabledAttr} ${titleAttr} /> Attended
        </td>
      </tr>
    `;
    })
...
function renderFilteredRegistrations() {
  ...
  if (selectedEventId) {
    filteredRegistrations = allRegistrations.filter(
      (r) => r.event_id == selectedEventId,
    );
    const event = allEvents.find((e) => e.event_id == selectedEventId);
    if (event && eventTitle)
      eventTitle.textContent = `Registrations for: ${event.title}`;
  } else {
